...
@router.get("/erp/v1/student/personal_info/{stu_KEY}", tags=["Student Management Version 1"], operation_id="get_student_personal_info_by_key_get")
def get_student_by_key_get(
    stu_KEY: int,
...
@router.get("/erp/v1/mcp/student/personal_info/{stu_ID}", tags=["Student Management Version 1"], operation_id="get_student_personal_info_by_admission_no")
def get_student_by_admission_no(
    stu_ID: str,
    db: Session = Depends(get_db)
):
    student = db.query(StudentPersonalInfo).filter(StudentPersonalInfo.stu_ID == stu_ID
                                                  ).first()
    if not student:
        return {"message": "Student not found"}
    return student
...
@router.post("/erp/v1/opac/list_books", tags=["Student Management Version 1"],operation_id="get_opac_books")
...
def get_exam_schedule(class_id: int, db: Session = Depends(get_db),
    ...
    query = (
        db.query(
            ...
            func.date_format(ExamScheduleDtl.dSchDate, "%d/%m/%Y").label("exam_date"),
            func.date_format(ExamScheduleDtl.sSchTime, "%h:%i %p").label("exam_time"),
            ...
            func.date_format(ExamScheduleHdr.dPublishingDate, "%d/%m/%Y").label("published_on"),
            case(
                (ExamScheduleHdr.dPublishingDate.is_(None), "Unpublished"),
                else_="Published"
        ...
        .filter(ExamScheduleHdr.iclass_ID == class_id)
        .order_by(Catlouge.catlg_description, ExamScheduleDtl.dSchDate)
    )

    results = query.all()

    return {"data": [dict(row._mapping) for row in results]}
...
def get_batchschedule(payload: BatchScheduleRequest, db: Session = Depends(get_db),
    ...
    dayorder = (
        db.query(B2ETblColAcdDayOrderProcess.adpsemno,
                 B2ETblColAcdDayOrderProcess.adpname,
                 AcademicYear.acdname)
        .join(AcademicYear, AcademicYear.acdyearid == B2ETblColAcdDayOrderProcess.adpacdid, isouter=True)         
        .filter(
            B2ETblColAcdDayOrderProcess.adpstart <= target_date,
            B2ETblColAcdDayOrderProcess.adpend >= target_date
        )
    ...
    adpsemno = dayorder.adpsemno 
    acdname = dayorder.acdname
    ...
    results = (
        db.query(TblColBatchSchedule.bsh_startdate,
                  TblColBatchSchedule.bsh_enddate )
...
@router.post(
    "/erp/v1/student/hourly_attendance",
    tags=["Student Management Version 1"],
    operation_id="get_StudentHourlyAttendance"
)
def get_hourly_attendance(
    ...
    # -------------------------------------------------
    # Subquery: Total Hours (vw_student_schedule)
    # -------------------------------------------------
    schedule_subq = (
        db.query(
    ...
    # -------------------------------------------------
    # Inner Query (matches raw SQL inner SELECT)
    # -------------------------------------------------
    inner_q = (
        db.query(
            ...
            func.sum(
                case(
                    (B2EStudent_detail.atd_status == "P", 1),
                    else_=0
                )
            ).label("attn"),
            func.sum(
                case(
                    (B2EStudent_detail.atd_status == "A", 1),
                    else_=0
                )
            ).label("absnt")
        )
        ...
        .join(
            B2EStudent_detail,
            TblColStuAtdHdr.attkey == B2EStudent_detail.atd_attkey
        )
        .join(
            StudentPersonalInfo,
            B2EStudent_detail.atd_stdid == StudentPersonalInfo.stu_KEY
        )
        .join(
            B2EClass,
            StudentPersonalInfo.stu_classid == B2EClass.class_id
        )
        .join(
            CourseBranch,
            B2EClass.cbid == CourseBranch.cbid
        )
        .outerjoin(
            B2EDepartment,
            CourseBranch.cdeptid == B2EDepartment.bcatid
        )
        .join(
            schedule_subq,
            schedule_subq.c.class_id == B2EClass.class_id
        )
        .filter(
    ...
    # -------------------------------------------------
    # Outer Query (final result with percent + late)
    # -------------------------------------------------
    final_q = (
        db.query(
            inner_q.c.class_id,
            ...
            (
                inner_q.c.tothrs - func.sum(inner_q.c.attn)
            ).label("late")
        )
        .group_by(
            inner_q.c.class_id,
...
@router.post("/erp/v1/student/consolidated_attendance", response_model=List[Consolidate_AttendanceResponse])
def get_consolidated_attendance(payload: Consolidate_AttendanceRequest, db: Session = Depends(get_db),
                                 current_user: str = Depends(get_current_user)
          ):

    # Parse sem_numbers from comma-separated string
    sem_numbers = [s.strip() for s in payload.bsh_semno.split(",")]

    # Convert start and end dates to datetime with hardcoded times
    batch_start = datetime.combine(payload.batch_startdate, datetime.min.time())
    batch_end = datetime.combine(payload.batch_enddate, datetime.max.time())
    
    # Step 1: Get current academic year id dynamically
    current_acdyear = db.query(AcademicYear.acdyearid).filter(
        AcademicYear.currentyear == '1'
    ).first()

    if not current_acdyear:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="No active current academic year found"
        )

    current_acdyearid = current_acdyear.acdyearid

    # Subquery to get adpstart from active academic year and semester type
    subquery = db.query(
        B2ETblColAcdDayOrderProcess.adpstart
    ).join(
        AcademicYear, B2ETblColAcdDayOrderProcess.adpacdid == AcademicYear.acdyearid
    ).filter(
        AcademicYear.status == 'Active',
        B2ETblColAcdDayOrderProcess.adpname == payload.adpname,
        B2ETblColAcdDayOrderProcess.adpacdid == current_acdyearid  # adjust dynamically if needed
    ).scalar_subquery()
    ...
    result = db.query(
        StudentPersonalInfo.stu_KEY,
        StudentPersonalInfo.stu_name,
        func.sum(cast(StuConAttDtl.catdnoofhrsatdper, Integer)).label("noofhrsatdper"),
        func.sum(cast(StuConAttDtl.catdnoofhrsatd, Integer)).label("noofhrsatd"),
        func.round(func.sum(
            case(
                (StuConAttDtl.catdtotalnoofhrs != None, cast(StuConAttDtl.catdtotalnoofhrs, Integer)),
                else_=cast(StuConAttHdr.catdnoofhrs, Integer)
            )
        )).label("noofhrs"),
        (
            func.round(func.sum(
                ...
                case(
                    (StuConAttDtl.catdnoofhrsatd != None, cast(StuConAttDtl.catdnoofhrsatd, Integer)),
                    else_=0
                )
            )
        ).label("absent")
    ).join(
        B2EClass, StudentPersonalInfo.stu_classid == B2EClass.class_id
    ).join(
        TblColBatchSchedule, B2EClass.class_id == TblColBatchSchedule.bsh_classid
    ).outerjoin(
        StuConAttDtl, StuConAttDtl.catd_stdid == StudentPersonalInfo.stu_KEY
    ).join(
        StuConAttHdr, StuConAttHdr.cattkey == StuConAttDtl.catd_attkey
    ).join(
        CourseBranch, B2EClass.cbid == CourseBranch.cbid
    ).outerjoin(
        B2EDepartment, CourseBranch.cdeptid == B2EDepartment.bcatid
    ).filter(
        StudentPersonalInfo.stu_statusflg != '3',
        StudentPersonalInfo.stu_KEY == payload.stu_KEY,
        StudentPersonalInfo.stu_classid == payload.class_id,
        StuConAttHdr.catdtodate >= batch_start,
        StuConAttHdr.catdtodate <= batch_end,
        TblColBatchSchedule.bsh_semno.in_(sem_numbers),
        TblColBatchSchedule.bsh_startdate >= subquery
    ).group_by(
        StudentPersonalInfo.stu_KEY,
        StudentPersonalInfo.stu_name
    ).all()
...
@router.post("/erp/v1/student/daily_attendance")
def get_daily_attendance(payload: Daily_AttendanceRequest, db: Session = Depends(get_db),
                         current_user: str = Depends(get_current_user)):

 
    base_date = datetime.strptime(payload.current_date, "%Y-%m-%d")
    ...
    query = (
    db.query(
        StudentPersonalInfo.stu_KEY.label("stu_key"),
        B2EClass.class_id.label("BAT_ID"),
        B2EClass.class_name,
        StudentPersonalInfo.stu_ID.label("stu_regno"),
        StudentPersonalInfo.stu_name.label("stu_name"),
        func.replace(TblColTtPeriod.ttp_dispname, "Hour", "").label("ATT_Session"),
        B2EStudent_detail.atd_status.label("PLA"),
        B2EStudent_detail.atd_subjectkey,
        TblColTtPeriod.ttp_dispname,
        TblIdSubject.SUB_NAME,
    )
    .select_from(TblColStuAtdHdr)   # 👈 force base table
    .join(B2EStudent_detail, TblColStuAtdHdr.attkey == B2EStudent_detail.atd_attkey)
    .join(StudentPersonalInfo, StudentPersonalInfo.stu_KEY == B2EStudent_detail.atd_stdid)
    .join(B2EClass, TblColStuAtdHdr.atdclassid == B2EClass.class_id)
    .join(TblColTtPeriod, TblColStuAtdHdr.atdperiod == TblColTtPeriod.ttp_id)
    .join(
        TblColBatchSchedule,
        and_(
            B2EClass.class_id == TblColBatchSchedule.bsh_classid,
            TblColBatchSchedule.bsh_startdate <= start_of_day,
            TblColBatchSchedule.bsh_enddate >= end_of_day,
            TblColStuAtdHdr.atdsem == TblColBatchSchedule.bsh_semno,
        ),
        isouter=True,
    )
    .join(CourseBranch, B2EClass.cbid == CourseBranch.cbid)   # 👈 explicit ON
    .join(B2EDepartment, CourseBranch.cdeptid == B2EDepartment.bcatid)  # 👈 explicit ON
    .join(TblIdSubject, TblIdSubject.Sub_Key == B2EStudent_detail.atd_subjectkey, isouter=True)  # 👈 explicit ON
    .filter(
        StudentPersonalInfo.statusflag.in_([0, 1]),
        TblColStuAtdHdr.atddate >= start_of_day,
        TblColStuAtdHdr.atddate <= end_of_day,
        TblColStuAtdHdr.atdstatus == "0",
        B2EClass.class_id == payload.class_id,
        StudentPersonalInfo.stu_KEY == payload.student_key,
    )
)
    ...
    response = [
        {
            ...
            "ATT_Session": row.ATT_Session,
            "PLA": row.PLA,
            "atd_subjectkey": row.atd_subjectkey,
            "ttp_dispname": row.ttp_dispname,
